...
public final class BuilderPlusTask extends Task {
  ...
  /**
   * Sets the CSS renaming map file, which must contain a JSON object
   * // TODO(cpeiset): update doc
   * or one of the following formats generated by Closure Stylesheets:
   *
   * <p><ul>
 ...
 CLOSURE_CSS_NAME_MAPPING = {
   ...
   * </ul></p>
   *
   * <p>The format used makes no difference, since in all cases the key-value
   * pairs are parsed into a {@link java.util.Map} and Builder Plus takes the
   * appropriate action as follows:</p>
   *
   * <p><ul>
   * <li><b>Builder Plus output mode: COMPILED</b></li>
   *   <li><ul>
   *     <li><b>Compilation level: SIMPLE or ADVANCED</b> - a temp file is
   *     created with a CSS renaming map formatted as shown above for
   *     CLOSURE_COMPILED. This file is then passed to the Closure Compiler
   *     immediately after base.js.</li>
   *     <li><b>Compilation level: WHITESPACE_ONLY</b> - a temp file is
   *     created with a CSS renaming map formatted as shown above for
   *     CLOSURE_UNCOMPILED. This file is then passed to the Closure Compiler
   *     as the first source file (before base.js).</li>
   *   </ul></li>
   * <li><b>Builder Plus output mode: RAW</b> - a CSS renaming map formatted
   * as shown above for CLOSURE_UNCOMPILED is prepended to the concatenated
   * output.</li>
   * </ul></p>
   *
  ...
  public void execute() {
    ...
    if (OutputMode.MANIFEST == this.outputMode) {
      if (this.outputManifest == null) {
        log(joiner.join(currentSources));
      }
    }
