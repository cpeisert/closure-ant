 ...
 *     </tr>
 * <tr class="altColor"><td><b>module</b></td><td>See {@link org.closureant.plovr.Module}
 *     </td></tr>
...
public final class Plovr extends Task {
  ...
  // TODO(cpeisert): add missing plovr config options such as
  // custom-externs-only
  ...
  /**
   * Adds a set of externs files. An {@literal <externsfileset>} nested
   * element is an Ant {@link org.apache.tools.ant.types.FileSet}, hence it
   * supports the same parameters as {@link org.apache.tools.ant.types.FileSet}.
   *
   * @param externsFiles A {@link FileSet} specifying externs files.
   */
  public void addConfiguredExternsFileSet(FileSet externsFiles) {
    List<File> files = AntUtil.getListOfFilesFromAntFileSet(
        getProject(), externsFiles);

    for (File file : files) {
      this.externs.add(file.getAbsolutePath());
    }
  }

  /**
   * Adds a list of externs files, where each file name may be separated by
   * whitespace and/or commas.
   *
   * @param externs A list of externs files.
   */
  public void addConfiguredExternsList(ExternsList externs) {
    this.externs.addAll(externs.getExterns());
  }
...
<soyfunctionplugins classNames="
  ...
  public void execute() {
    ...
    log("Creating plovr config file: " + this.configFile.getAbsolutePath(),
        Project.MSG_INFO);
    Config config = getPlovrConfigOptions();

    Gson gson = new GsonBuilder()
        .setPrettyPrinting()
        .registerTypeAdapter(ExperimentalCompilerOptions.class,
            config.experimentalCompilerOptions.new Serializer())
        .registerTypeAdapter(CompileTimeDefines.class,
            config.define.new Serializer())
        .registerTypeAdapter(ModuleCollection.class,
            config.modules.new Serializer())
        .create();

    // Exclude unset nested elements from appearing in the config file.
    config.nullifyEmptyCollections();
    String currentPlovrConfig = gson.toJson(config);
    ...
    if (PlovrMode.CONFIG != this.plovrMode) {
      ...
      if (PlovrMode.SERVE == this.plovrMode) {
        ...
        try {
          currentSources = getCurrentSources(config);
        } catch (IOException e) {
          throw new BuildException(e);
        }
   ...
   * {@link Config}. Paths are recursively scanned matching all files
  ...
  /**
   * Initialize a {@link Config} instance based on the Ant attribute and
   * nested element settings for this task.
   *
   * @return {@link Config} initialized based on the task settings
   * @throws BuildException on error
   */
  private Config getPlovrConfigOptions() {
    Config config;

    if (this.compilerOptions == null) {
      config = new Config();
    } else {
      config = this.compilerOptions.toPlovrConfig(getProject());
    }
    ...
    if (this.closureLibrary != null) {
      try {
        ...
        config.closureLibrary = baseJs.getParent();
      } catch (IOException e) {
        throw new BuildException(e);
      }
    }
    ...
    if (this.inherits != null) {
      config.inherits = this.inherits.getAbsolutePath();
    }
    if (this.experimentalExcludeClosureLibrary != null) {
      config.experimentalExcludeClosureLibrary =
          this.experimentalExcludeClosureLibrary;
    }
    if (this.exportTestFunctions != null) {
      config.exportTestFunctions = this.exportTestFunctions;
    }
    if (this.fingerprint != null) {
      config.fingerprint = this.fingerprint;
    }
    config.globalScopeName = this.globalScopeName;
    config.jsdocHtmlOutputPath = this.jsdocHtmlOutputPath;
    config.moduleInfoPath = this.moduleInfoPath;
    config.moduleOutputPath = this.moduleOutputPath;
    config.moduleProductionURI = this.moduleProductionURI;
    if (this.outputFile != null) {
      config.outputFile = this.outputFile.getAbsolutePath();
    }
    if (this.testTemplate != null) {
      config.testTemplate = this.testTemplate.getAbsolutePath();
    }

    // Add nested elements. Note: <compiler> already added.

    if (!this.externs.isEmpty()) {
      config.externs.addAll(this.externs);
    }
    ...
    for (FileSet input : inputs) {
      ...
      for (File inputFile : inputFiles) {
        config.inputs.add(inputFile.getAbsolutePath());
      }
    }
    ...
    for (File dir : rootDirectories) {
      config.paths.add(dir.getAbsolutePath());
    }
    ...
    for (FileSet source : sources) {
      ...
      for (File sourceFile : sourceFiles) {
        config.paths.add(sourceFile.getAbsolutePath());
      }
    }

    for (String soyFunctionPlugin : this.soyFunctionPlugins) {
      config.soyFunctionPlugins.add(soyFunctionPlugin);
    }

    for (StringNestedElement testExclude : this.testExcludes) {
      config.testExcludes.add(testExclude.getValue());
    }
    return config;
  }
}
